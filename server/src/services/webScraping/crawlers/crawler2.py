...
@traceable(run_type="tool", name="Crawl")
def crawl(url, data_points, links_scraped):
    """
    Crawl a website starting from a given URL and extract structured data from each page you can find.
    
    Args:
    url (str): The URL to begin crawling from.
    data_points (List[Dict]): The list of data points to extract.
    links_scraped (List[str]): List of already scraped or crawled links.
    ...
    try:
        ...
        #             "waitFor": 3000,  # wait for a second for pages to load
        ...
        crawled_data = app.crawl_url(
            ...
            params={
                ...
                'scrapeOptions': {
                    'formats': [ 'markdown' ],
                    'excludeTags': [ '#ybar', '#sports-module-scorestrip', "#ad", ".advertisement", ".sponsored-content"],
        ...
        # print('crawled_data data: ', crawled_data["data"])
        # print('crawled_data for one entry: ', crawled_data["data"][0])
        # print('crawled_data for one entry markdown: ', crawled_data["data"][0]["markdown"])

        markdown = ""
        counter = 0
        for item in crawled_data["data"]:
            markdown += item["markdown"][: (MAX_TOKEN * 2)]
            # print(f'item for {counter}: {item["markdown"]}')
            counter += 1
            # print('markdown: ', markdown)



        # markdown = crawled_data["markdown"][: (MAX_TOKEN * 2)]
        links_scraped.append(url)
        ...
        print("Unable to crawl the url")
        print(f"Exception: {e}")
        return "Unable to crawl the url"    
...
def website_search(entity_name: str, website: str, data_points, links_scraped):
    ...
    tools = [
        {
            ...
            "function": {
                "name": "crawl",
                "description": "Crawl a URL for information",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "url": {
    ...
    if len(data_keys_to_search) > 0:
        ...
        You will crawl pages within a company/entity's domain to find specific data about the company/entity, but You NEVER make up links, ONLY scrape real links you found or are given. For this specific task, you will be given a website to crawl, and you will crawl the website to find relevant data points. Here are some special instructions:

        {special_instruction}

        You only use data retrieved from the crawler, do not make things up;
...
def run_research(entity_name, website, data_points):
    ...
    print('data_points1: ', data_points)
    ...
    print('data_points2: ', data_points)
...
# Test Run
entity_name = "YahooSports"
website = "https://sports.yahoo.com/"
# special_instruction = "This is a weather news website, you should crawl the website to find the latest weather information for various locations. Look for as much data as possible, but don't scrape the same url twice. There are many links to stories as well as categories for articles broken down by topic. You should crawl each story section to get comprehensive coverage of articles from each weather related topic."

special_instruction = "This is a sports news website, you should crawl the website to find the latest news about various sports. Look for as much data as possible, but don't scrape the same url twice. There are many links to articles as well as categories for articles broken down by sport. You should crawl each sport section to get comprehensive coverage of articles from each sport. The most important data points are the articles, so you should focus on finding the articles and their content."
...
# "This is a website of sports news, you should crawl the website to find the latest news about various sports. Look for as much data as possible, but don't scrape the same url twice. There are many links to articles as well as categories for articles broken down by sport. You should crawl each sport section to get comprehensive coverage of articles from each sport."